  ...
  if (authToken) {
    authModel.findOne({ authToken })
        ...
        if (check.isEmpty(authDetails)) {
          ...
          token.verifyClaim(authDetails.authToken)
            ...
            .catch((error) => {
              ...
              const apiResponse = response.generate(true, 'failed to authorize', 500, null);
      ...
      .catch((error) => {
        ...
        const apiResponse = response.generate(true, 'failed to verify authToken', 500);
