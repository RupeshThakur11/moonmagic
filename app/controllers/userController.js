...
const signUp = (req, res) => {
  validateEmail = () => new Promise((resolve, reject) => {
    if (req.body.email) {
      if (check.isEmpty(req.body.password)) {
        const apiResponse = response.generate(true,
  ...
  const createUser = () => new Promise((resolve, reject) => {
    ...
    const bcryptPassword = password.hashPassword(
      req.body.password,
    );
    userModel.findOne({ email: req.body.email })
      .then((data) => {
        if (check.isEmpty(data)) {
          ...
          const apiResponse = response.generate(false,
...
const login = (req, res) => {
  ...
  const verifyUser = userData => new Promise((resolve, reject) => {
    password.comparePassword(req.body.password, userData.password)
      .then((isValid) => {
        if (isValid) {
          const details = userData.toObject();
          delete details.__v;
          delete details._id;
  ...
  const genToken = userDetails => new Promise((resolve, reject) => {
  ...
  const saveToken = tokenDetails => new Promise((resolve, reject) => {
    authModel.findOne({ userId: tokenDetails.userId })
      .then((tokenDetail) => {
        if (check.isEmpty(tokenDetail)) {
          const newAuthToken = new authModel({
...
const forgotPassword = (req, res) => {
  ...
  // resetUrl
  const generateResetUrl = (userDetail) => {
      ...
      if (err) {
        ...
        newResetToken.save()
          .then((resetDetail) => {
            ...
            const resetEmail = `<a href='http://localhost:${appConfig.port}${baseUrl}/resetPassword/${userDetail.email}/${token}'>click here</a>`;
            nodemailer.mailer({
  ...
  resetTokenModel.findOne({ $and: [{ email: req.params.email }, { token: req.params.token }] })
    .then((userdetail) => {
      if (userdetail) {
        ...
        console.log(now);
        const expire = Date.parse(userdetail.expiresOn);
        console.log(expire);
        ...
        const newToken = userdetail.token;
  ...
  if (check.isEmpty(req.body.email) || check.isEmpty(req.body.password) || check.isEmpty(req.body.token)) {
    ...
    resetTokenModel.findOne({ $and: [{ email: req.body.email }, { token: req.body.token }, { confirmed: true }] })
      .then((userdetail) => {
        ...
        const updatedPassword = password.hashPassword(req.body.password);
        const update = {
      ...
      .catch((error) => {
        console.log(error);
        const apiResponse = response.generate(true, 'failed to verify token', 401, null);
