var multiLevelPlansModel = require('../models/multiLevelPlansModel.js');
...
module.exports = {
    ...
    list: function(req, res) {
        multiLevelPlansModel.find(function(err, multiLevelPlanss) {
            if (err) {
    ...
    create: function(req, res) {
        var multiLevelPlans = new multiLevelPlansModel({
        ...
        multiLevelPlans.save(function(err, multiLevelPlans) {
            if (err) {
                return res.status(500).json({
    ...
    remove: function(req, res) {
        var id = req.params.id;
        multiLevelPlansModel.findByIdAndRemove(id, function(err, multiLevelPlans) {
            if (err) {
                return res.status(500).json({
    ...
    calculatePlans: function(req, res) {
        var id = req.body.id;
        multiLevelPlansModel.findOne({
            _id: id
        }, function(err, multiLevelPlans) {
            ...
            if (multiLevelPlans) {
                ...
                if ((multiLevelPlans.stars === 'star' ||  multiLevelPlans.level === 1) && multiLevelPlans.amount >= 1000)  {
                      let deductedAmountForSilver = multiLevelPlans.rewardPrize - 500;
                    ...
                    if (multiLevelPlans.rewardPrize >= 5000) {
                        let deductedAmountForGold = multiLevelPlans.rewardPrize - 2500;
                    ...
                    if (multiLevelPlans.rewardPrize >= 25000) {
                        let deductedAmountForGold = multiLevelPlans.rewardPrize - 12500;
                    ...
                    if (multiLevelPlans.rewardPrize >= 125000) {
                        let deductedAmountForGold = multiLevelPlans.rewardPrize - 62500;
